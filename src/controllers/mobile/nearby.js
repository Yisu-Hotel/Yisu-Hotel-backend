const nearbyController = {
  ...
  getHotelNearbyInfo: async (req, res) => {
    try {
      ...
      const { type, radius = 3000 } = req.query;
      ...
      console.error('获取酒店周边信息错误:', error);
  ...
  getNearbyAttractions: async (req, res) => {
    try {
      const { hotel_id } = req.params;
      const { page = 1, page_size = 10, radius = 5000 } = req.query;
